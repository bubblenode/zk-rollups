...
use zksync_types::{
    ...
    Address, L1BlockNumber, Nonce, EIP_1559_TX_TYPE, EIP_4844_TX_TYPE, H256, U256, LEGACY_TX_TYPE
};
        ...
        signing_gateway
            .sign_prepared_tx_for_addr(
                ...
                Options::with(|opt| {
                    ...
                    // };
                    opt.gas_price = Some(U256::from(base_fee_per_gas + priority_fee_per_gas));
                    opt.transaction_type = Some(LEGACY_TX_TYPE.into());
